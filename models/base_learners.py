...
class BaseLearnerStack:
    def __init__(self):
        self.cat = CatBoostRegressor(depth=3, l2_leaf_reg=15.0, min_data_in_leaf=8, iterations=300, learning_rate=0.05, random_seed=42, verbose=0)
        self.lgb = LGBMRegressor(max_depth=3, min_child_samples=10, reg_lambda=15.0, n_estimators=300, learning_rate=0.05, random_state=42, verbose=-1)
        self.xgb = XGBRegressor(max_depth=3, min_child_weight=8, reg_lambda=15.0, n_estimators=300, learning_rate=0.05, random_state=42, verbosity=0)
    ...
    def generate_oof(self, X, y_home, y_away):
        ...
        for train_idx, test_idx in cv.split(X):
            ...
            for model in [self.cat, self.lgb, self.xgb, self.ridge]:
                model.fit(X_train, y_train_h)
                p_home += model.predict(X_test)
            oof_home[test_idx] = p_home / 4.0
